...
class LLMFL(FL):
    search_str_with_file_promt_template = """
### search results ###
...
file2.py
```
Only return the files. Do not include any comments or explanations.
"""
...
Please provide either the class, the function name or line numbers that need to be edited. If you need to edit multiple places, please provide all of them. But if you want to provide the line number, please give me a number in the middle every time.
...
Please provide the method, or function names that need to be edited.
Here is the format you need to follow, don't forget the "```":
### Examples:
```
full_path1/file1.py
...
"""
    ...
    def _parse_model_return_lines(self, content: str) -> list[str]:
        if content:
            return [line.split("#")[0].strip() for line in content.strip().split("\n")]
        return []
    ...
    def search_in_problem_statement(self, reproduce_info) -> dict[str, str]:
        ...
        if self.backend == "openai":
            ...
            traj = tool_model.codegen(message, num_samples=1,
                                 tools=[search_string_schema, search_class_def_schema, search_func_def_schema])[0]
    ...
    def localize(
        self, top_n=1, mock=False, match_partial_paths=False,  search_res_files=None, num_samples=1, coverage_info=None, additional_info=None
    ):
        ...
        if search_str_with_files:
            search_str_with_file_prompt = self.search_str_with_file_promt_template.format(search_results=search_str_with_files)
    ...
    def localize_function_from_compressed_files(self, file_names, mock=False, num_samples=1, coverage_info=None, additional_info=None):
        from patchpilot.util.api_requests import num_tokens_from_messages
        from patchpilot.util.model import make_model        
        ...
        return model_found_locs_merged_result, {"raw_output_loc": raw_outputs}, traj
